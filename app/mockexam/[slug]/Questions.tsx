...
export default function Page() {
  ...
  let checkBox : number = 0; //For check the right ans and handle submit for history

  // const [currentTime , setCurrentTime] = useState<number>()
  let historyRecored : string;
  let currentTime = 0;
  ...
  function handlesubmit(): void {
    historyRecored = historyToString();
    console.log(historyRecored);
    ...
    if (index == randomMathItems.length - 1) {
      if (checkScore) {
        ...
        addHistory(checkBox,randomMathItems[index].picquestions.id)
        console.log(true);
      } else {
        addHistory(checkBox,randomMathItems[index].picquestions.id)
        submitForm();
